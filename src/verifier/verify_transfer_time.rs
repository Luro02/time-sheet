...
pub struct InvalidTransferTime {
    expected_working_time: WorkingDuration,
    total_time: WorkingDuration,
...
impl Verifier for VerifyTransferTime {
    ...
    fn verify(&self, config: &Config) -> Result<(), Self::Errors> {
        ...
        let transfer_to_next_month = config.month().transfer().next();
        let expected_working_duration = config.month().expected_working_duration();
        ...
        if transfer_to_next_month + expected_working_duration != total_time {
            return Err(iter::once(InvalidTransferTime {
                expected_working_time: expected_working_duration,
                total_time,
                transfer_time: transfer_to_next_month,
            }));
        }
